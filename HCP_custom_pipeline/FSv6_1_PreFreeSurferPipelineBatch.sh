...
main()
{
	...
	for Subject in $Subjlist ; do
		...
		numT1ws=$(/bin/ls -d ${StudyFolder}/${Subject}/unprocessed/T1w? | wc -l)	
		...
		while [ $i -le $numT1ws ] ; do
			...
			T1wInputImages=$(echo "${T1wInputImages}${StudyFolder}/${Subject}/unprocessed/T1w${i}/${Subject}_T1w${i}.nii.gz@")		
			i=$(($i+1))
		done
		...
		numT2ws=$(/bin/ls -d ${StudyFolder}/${Subject}/unprocessed/T2w? | wc -l)		
		...
		while [ $i -le $numT2ws ] ; do
			...
			T2wInputImages=$(echo "${T2wInputImages}${StudyFolder}/${Subject}/unprocessed/T2w${i}/${Subject}_T2w${i}.nii.gz@")
			i=$(($i+1))
		done
		...
		# Hires T1w MNI template
		T1wTemplate="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_1mm_t1w.nii.gz"

		# Hires brain extracted MNI template
		T1wTemplateBrain="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_1mm_t1w_brain.nii.gz" 

		# Lowres T1w MNI template
		T1wTemplate2mm="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_2mm_t1w.nii.gz" 

		# Hires T2w MNI Template
		T2wTemplate="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_1mm_t2w.nii.gz" 

		# Hires T2w brain extracted MNI Template
		T2wTemplateBrain="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_1mm_t2w_brain.nii.gz" 

		# Lowres T2w MNI Template
		T2wTemplate2mm="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_2mm_t2w.nii.gz" 

		# Hires MNI brain mask template
		TemplateMask="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_1mm_brain_mask.nii.gz"

		# Lowres MNI brain mask template		
		Template2mmMask="${HCPPIPEDIR_Templates}/nihpd_asym_04.5-18.5_2mm_brain_mask_fillh_dil1.nii.gz"
